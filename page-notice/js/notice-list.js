...
function getList(pageNum) {
	pageCount = pageNum;
	var title = document.getElementById("searchKey").value;
	var userId = plus.storage.getItem("sysUsersId");
	var token = plus.storage.getItem("token");
	
	var url = topUrl + 'noticeUser/pageList';
	var listHtml = '';
	mui.ajax(url, {
		data: {
			token: token,
		...
		success: function(data) {
			...
			for (var i = 0; i < rows.length; i++) {
				if (rows[i].userName.length > 2) {
					rows[i].userName = rows[i].userName.substr(0, 1);
				}
				var noticeLisrColor = refactorChangeColor(i);
				var html = '<li showType=' + rows[i].showType + ' id=' + rows[i].noticeInfoId + ' class="mui-table-view-cell">'
				html += '<a href="javascript:;">';
				html += '  <span class="mui-media-object mui-pull-left xingm ' + noticeLisrColor +
					'" style="padding:0px;text-align:center">' + rows[i].userName + '</span>';
...
mui("#noticeList").on('tap', '.mui-table-view-cell', function() {
	...
	var url = '../page-notice/main-carouselDetail.html';
	if (showType == 3) {
		url = '../page-notice/notice-content.html'
	}
	openNewPage(url, id);
})
...
function createList() {
	...
	if (length > 0) {
		...
		for (var i = 0; i < length; i++) {
			...
			var html = '';
			html += '<a href="javascript:;">';
			html += '  <span class="mui-media-object mui-pull-left xingm badgeSpan" style="padding:0px 0px 0px 5px">' +
				getSource(userName) + '</span>';
