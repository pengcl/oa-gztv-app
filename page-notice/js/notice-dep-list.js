...
function getList(pageNum) {
	...
	var args = {
		...
		// pageSize : 15,	
		size : 15,
		//bulletinReadStatus:0,
	};
	var _url = URL_NOTICE_LIST ;//"bulletinRead/pageListView"; //topUrl + URL_NOTICE_LIST; // 
	sendRequest(_url, args, function(rtnData) {
		
		console.log("--------------------------------------");
		console.log("公告列表: " + JSON.stringify(rtnData));
		if (pageNum == 1) { // 若不为追加数据, 则清空列表数据
			noticeListData = [];
			table.innerHTML = "";
			itemCount = 0;
			maxCount = 0;
