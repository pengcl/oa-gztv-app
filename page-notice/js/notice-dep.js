...
var subpages = ['notice-dep-list-cp.html', ''];
var subpage_style = {
	top: subpageTop, //90px
	bottom: '0px'
};
...
//获取部门专栏信息
function getDepNoticeType(){
	var url = topUrl + 'noticeType/myDeptNoticeType.action?token=' + token;
	var searchInput = document.getElementById("searchInput");
	mui.ajax(url, {
		success: function(data) {
			console.log(JSON.stringify(data));
			var innerHtml='<option value="">全部</option>';
			for(var i=0;i<data.length;i++){
				var html='<option value="'+data[i].a_attr.id+'">'+data[i].a_attr.name+'</option>'
				innerHtml+=html;
			}
